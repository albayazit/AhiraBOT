...
async def qoran_last_ten_get(callback: types.CallbackQuery):
	...
	await callback.answer()	
	await callback.message.delete()
	await bot.send_photo(callback.from_user.id, last_ten_id[data], caption = surah_translate[data], reply_markup= await client_kb.markup_surah(data))
